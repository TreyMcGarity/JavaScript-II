...
let fullNames = [];
runners.forEach((person) => {fullNames.push(person["first_name"]) + " " + fullNames.push(person["last_name"]);});
console.log(fullNames);

// ==== Challenge 2: Use .map() ====
// The event director needs to have all the runners' first names in uppercase because the director BECAME DRUNK WITH POWER. Populate an array called `firstNamesAllCaps`. This array will contain just strings.
let firstNamesAllCaps = [];
runners.forEach((person) => {firstNamesAllCaps.push(person["first_name"]);});
firstNamesAllCaps = firstNamesAllCaps.map(names => names.toUpperCAse()); 
